...
def verify_test_files(path, block_size, num_blocks):
    ...
    print(f"\n✅ Kiểm tra xong {max_good_block} block thành công!\n")
    if max_good_block < num_blocks:
        real_capacity = (block_size * max_good_block) / (1024 ** 2)
        print(f"⚠️ Thẻ nhớ có thể bị FAKE! Dung lượng ghi được an toàn: {real_capacity:.2f} MB")
    else:
        print("✅ Không phát hiện lỗi dữ liệu. Có thể là thẻ thật.")
def cleanup(path):
